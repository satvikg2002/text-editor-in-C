...
#define HLDB_ENTRIES (sizeof(HLDB) / sizeof(HLDB[0])) // highlighter database
...
void editorUpdateSyntax(erow *row)
{
    ...
    int prev_sep = 1;
    int in_string = 0;
    int in_comment = (row->idx > 0 && E.row[row->idx - 1].hl_open_comment); // check if prev row is ML_COMMENT
    ...
    while (i < row->rsize)
    {
        ...
        if (mcs_len && mce_len && !in_string)
        {
            if (in_comment)
            {
                ...
                if (!strncmp(&row->render[i], mce, mce_len)) // in the middle of the comment
                {
        ...
        if (prev_sep)
        {
            ...
            if (keywords[j] != NULL) // check if for loop was broken
            {
                prev_sep = 0;
                continue;
            }
    ...
    if (changed && row->idx + 1 < E.numrows)
        editorUpdateSyntax(&E.row[row->idx + 1]);   // sync the inclusion of lines in ml comment
}
...
void editorSelectSyntaxHighlight()
{
    ...
    for (unsigned int j = 0; j < HLDB_ENTRIES; j++)
    {
        ...
        while (s->filematch[i])
        {
            int is_ext = (s->filematch[i][0] == '.'); // ith extension in HLDB
            if ((is_ext && ext && !strcmp(ext, s->filematch[i])) ||
                (!is_ext && strstr(E.filename, s->filematch[i]))) // check for matching extension
            {
                E.syntax = s;
